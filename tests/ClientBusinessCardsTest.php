...
class ClientBusinessCardsTest extends ClientTestCase
{
    public function test_process_business_card_base64(): void
    {
        ...
        $json_response = json_decode($veryfi_client->process_business_card_base64($file), true);
    ...
    public function test_process_business_card_url(): void
    {
        ...
        $url = 'https://cdn-dev.veryfi.com/testing/veryfi-python/business_card.jpg';
        $json_response = json_decode($veryfi_client->process_business_card_from_url($url), true);
        $this->assertNotEmpty($json_response['id']);
    }

    public function test_get_business_cards(): void
    {
        if ($this->mock_responses) {
            $veryfi_client = $this->getMockBuilder(Client::class)
                ->onlyMethods(['exec_curl'])
            ...
            $file_path = __DIR__ . '/resources/getBusinessCards.json';
            $file = fopen($file_path, 'r');
            $file_data = mb_convert_encoding(fread($file, filesize($file_path)), 'UTF-8');
            $veryfi_client->expects($this->atLeastOnce())
                ->method('exec_curl')
        ...
        $json_response = json_decode($veryfi_client->get_business_cards(), true);
        ...
        $json_response = json_decode($veryfi_client->get_business_card($document_id), true);
        $this->assertNotEmpty($json_response);
    }
    public function test_delete_business_card(): void
    {
        if ($this->mock_responses) {
            $veryfi_client = $this->getMockBuilder(Client::class)
                ->onlyMethods(['exec_curl'])
        ...
        $delete_json_response = json_decode($veryfi_client->delete_business_card($document_id));
