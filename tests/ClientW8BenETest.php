...
class ClientW8BenETest extends ClientTestCase
{
    public function test_process_w8bene(): void
    {
        if ($this->mock_responses) {
            ...
            $file_path = __DIR__ . '/resources/processW8BenE.json';
        ...
        $json_response = json_decode($veryfi_client->process_w8bene_base64($file), true);
        $this->assertNotEmpty($json_response['id']);
    }

    public function test_process_w8bene_url(): void
    {
        if ($this->mock_responses) {
            $veryfi_client = $this->getMockBuilder(Client::class)
                ->onlyMethods(['exec_curl'])
        ...
        $url = 'https://cdn-dev.veryfi.com/testing/veryfi-python/w8bene.jpg';
        $json_response = json_decode($veryfi_client->process_w8bene_from_url($url), true);
        $this->assertNotEmpty($json_response['id']);
    }

    public function test_get_w8benes(): void
    {
        if ($this->mock_responses) {
            $veryfi_client = $this->getMockBuilder(Client::class)
                ->onlyMethods(['exec_curl'])
            ...
            $file_path = __DIR__ . '/resources/getW8BenEs.json';
            $file = fopen($file_path, 'r');
            $file_data = mb_convert_encoding(fread($file, filesize($file_path)), 'UTF-8');
            $veryfi_client->expects($this->atLeastOnce())
                ->method('exec_curl')
        ...
        $json_response = json_decode($veryfi_client->get_w8benes(), true);
        ...
        $json_response = json_decode($veryfi_client->get_w8bene($document_id), true);
        $this->assertNotEmpty($json_response);
    }
    public function test_delete_w8bene(): void
    {
        if ($this->mock_responses) {
            $veryfi_client = $this->getMockBuilder(Client::class)
                ->onlyMethods(['exec_curl'])
        ...
        $delete_json_response = json_decode($veryfi_client->delete_w8bene($document_id));
